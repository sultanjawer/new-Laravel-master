...
$.extend( DataTable.ext.classes, {
	...
	sLengthSelect: "custom-select custom-select-sm form-control form-select-sm",
...
$.extend(true, $.fn.dataTableExt.oStdClasses, {
	...
	"sLengthSelect": "form-control form-select-sm custom-select custom-select-sm"
});	
...
var _modal = $(
	'<div class="modal fade dtr-bs-modal" tab-index="-1" role="dialog">'+
		...
		if ( ! $.fn.modal ) {
			...
			if ( ! update ) {
				if ( options && options.header ) {
					...
					header
						.empty()
						.append( '<h5 class="modal-title"><i class="subheader-icon fal fa-file-invoice mr-1"></i>'+options.header( row )+'</h5>' )
						.append( button );
				}
				...
				for (var j in columnDefs) {
					...
					if (columnDefs[j].type.indexOf("hidden") >= 0) {
						...
						if (columnDefs[j].type.indexOf("select") >= 0) {
							...
							data += "<select class='custom-select custom-select-sm form-control form-select-sm" + (columnDefs[j].select2 ? ' select2' : '') + "' id='" + columnDefs[j].name + "' name='" + columnDefs[j].title + "' " + (columnDefs[j].multiple ? 'multiple' : '') + ">" + options
								+ "</select>";
						}
				...
				for (var j in columnDefs) {
					if (columnDefs[j].type.indexOf("hidden") >= 0) {
						...
						if (columnDefs[j].type.indexOf("select") >= 0) {
							...
							data += "<select class='custom-select form-control form-select-sm" + (columnDefs[j].select2 ? ' select2' : '') + "' id='" + that._quoteattr(columnDefs[j].name) + "' name='" + that._quoteattr(columnDefs[j].title) + "' " + (columnDefs[j].multiple ? 'multiple' : '') + ">" + options
								+ "</select>";
						}
						data += "</div><div style='clear:both;'></div></div>";
					}
